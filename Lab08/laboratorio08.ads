...
package Laboratorio08 is
   ...
   -- F1 y F2 son dos fechas v�lidas
   ...
   --      la posicion de insercion estar� en el rango
   --      que el numero de elementos que contenga la lista +1
   --      V tiene al menos un hueco libre
   -- post: si pos es una posici�n valida de datos, V incluye num en dicha posicion
   ...
   -- post: Resultado = numero m�ximo de los incluidos en V
   procedure Ordenar_Seleccion (V: in out T_Vector_Enteros);
   -- pre:  El Vector tiene todos los elementos ocupados
   -- post: Los elementos del vector est�n ordenados
   ...
   -- descomentando use, estar�a disponible el tipo Vector:
   -- use P_Vector_Integer;
   --
   -- Tambi�n podemos renombrar el tipo:
   ...
   procedure Inicializar_Matriz_Espiral(M: in out T_Matriz);
   -- pre: M es una matriz sin inicializar
   -- post: M esta inicializada empezando por el elemento central, comenzando en 1,
   --       y empezando en las 3 (horario) en contra de las agujas del reloj (ver enunciado)
   procedure Inicializar_Matriz_Espiral_Adyacentes(M: in out T_Matriz);
   -- pre: M es una matriz sin inicializar
   -- post: M esta inicializada empezando por el elemento central, comenzando en 1,
   --       empezando en las 3 (horario) en contra de las agujas del reloj y
   --       teniendo en cuenta los valores adyacentes inicializados previamente �nicamente

   subtype T_Segmentos is Character range 'a'..'g';
   type T_Displays is array(1..10, T_Segmentos) of Boolean;
   type T_Columnas is array (T_Segmentos) of T_Segmentos;
   procedure ENIGMA_DISPLAY(Displays: in T_Displays; Columnas: out T_Columnas);
   -- pre: Display es una matriz con 10 displays (1 por fila) y
   --      siete valores booleanos (1 por segmento).
   --      Cada fila tiene un display distinto y todos usan la misma columna para un segmento
   -- post: Una lista en la que se indica la columna de T_display que representa cada segmento
   --   Columnas('d') tiene la columna de Displays que representa al segmento g (en el ejemplo la
   --      Por ejemplo Columnas = ('a'=>'a','b'=>'b',c=>'c','d'=>'g','e'=>'d','f'=>'e','g'=>'f')

   procedure Relojes_De_Arena(M: in T_Matriz; Max: out Integer);
   -- pre: M es una matriz bidimensional
   -- post: Max contiene el maximo de todos los relojes de arena que se pueden incluir en la matriz
