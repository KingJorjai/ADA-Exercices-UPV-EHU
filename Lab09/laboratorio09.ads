package Laboratorio09 is
   ...
   --post: F es la fecha del dia siguiente a la valor dado de F
   ...
   -- C es un complejo en forma trigonometrica
   ...
   -- pre:  la posicion de insercion estara en el rango
   --      que el numero de elementos que contenga la lista +1
   --      L tiene al menos un hueco libre
   -- post: si pos es una posicion valida de datos, L incluye num en dicha posicion
   ...
   -- post: Resultado = numero maximo de los incluidos en L
   procedure Ordenar_Burbuja(L: in out T_Lista_Estatica);
   -- post: Los elementos de L estan ordenados
   ...
   --post: Fil = fila en la que esta Num (0 si no esta)
   ...
   --      Fila = la menor Fila en la que este maximo
   --      Columna = la menor columna en la que este maximo en esa fila

  --No hacer 9.13 ni 9.14
   procedure Ordenar_Quicksort(L: in out T_Lista_Estatica);
   --Ordena la lista estatica mediante Quicksort

   function Patron(N: in Positive) return T_Matriz;
   --Post: Resultado es una matriz de orden N inicializada segun enunciado

   function Patron_Espiral(N: in Positive) return T_Matriz;
   --Post: Resultado es una matriz de orden N inicializada segun enunciado
